...
export default function RatingFilter() {
  ...
  const handleMetacriticSelected = useActiveFilter(
    (state) => state.handleMetacriticSelected,
  );
  const percentage = (metacriticSelected / 100) * 100;

  // Converter metacritic (0-100) para estrelas (0-5)
  const starsValue = (metacriticSelected / 100) * 5;
      ...
      <div className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/30 p-1">
        <div className="relative">
          <input
            ...
            max="100"
            step="1"
            value={metacriticSelected}
            onChange={(e) => handleMetacriticSelected(Number(e.target.value))}
        ...
        <div className="flex flex-col items-center justify-between gap-2">
          <span className="text-sm font-medium text-slate-400">
            {metacriticSelected === 0 ? "Any rating" : "Minimum rating"}
          </span>
          ...
          <div className="flex min-h-[63px] w-full items-center gap-2 rounded-lg border border-yellow-500/30 bg-gradient-to-br from-yellow-500/10 via-orange-500/10 to-yellow-500/5 px-4 py-2 shadow-lg shadow-yellow-500/10">
            ...
            <div className="ml-2 flex items-center gap-1 border-l border-yellow-500/30 pl-3">
